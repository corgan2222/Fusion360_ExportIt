...
class LazyDocument:
    ...
    def open(self):
        ...
        if self._ctx.unhide_all and self.file.fileExtension != 'f2d':
            try:
                ...
                log(f'Warning: unhide_all failed, continuing anyway\n{traceback.format_exc()}')
    ...
    def close(self):
        ...
        try:
            ...
            log(f'Warning: error while closing document\n{traceback.format_exc()}')
    ...
    with LazyDocument(ctx, file) as doc:
        ...
        if ctx.save_sketches and file.fileExtension != 'f2d':
            try:
                ...
                counter.errored += 1
                log(f'Error exporting sketches\n{traceback.format_exc()}')
    ...
    if recurse:
        for sub_folder in folder.dataFolders:
            try:
                ...
                log(f'Got exception visiting subfolder {sub_folder.name}\n{traceback.format_exc()}')
                counter.errored += 1
...
def main(ctx: Ctx) -> Counter:
    ...
    if ctx.use_active_folder:
        ...
        for project_id, folder_ids in ctx.projects_folders.items():
            ...
            if project is None:
                log(f'ERROR: project with id "{project_id}" not found — check your projects_folders config')
                counter.errored += 1
                continue
